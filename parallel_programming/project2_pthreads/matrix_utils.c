...
//Creates a rows x cols matrix with "random" positive entries between 0 and 9
...
double* matrix_multiply(double* m1, int m1_rows, int m1_cols, double* m2, int m2_rows, int m2_cols) {
    if (m1_cols != m2_rows) //not possible to multiply
        return NULL;
    double* matrix = malloc(m1_rows * m2_cols * sizeof(double));
    int i, j;
    for (i = 0; i < m1_rows; i++) {
        for (j = 0; j < m2_cols; j++) {
            int sum = 0;
...
void read_two_matrices_from_file(char* filename, double** m1, int* m1_rows, int* m1_cols, double** m2, int* m2_rows, int* m2_cols) {
    ...
    double *matrix = malloc((*m1_rows) * (*m1_cols) * sizeof(double));

    int i, j;
    for (i = 0; i < *m1_rows; i++) {
        for (j = 0; j < *m1_cols; j++) {
            fscanf(fp, "%lf", &matrix[j + i*(*m1_cols)]);
        }
    }
    ...
    *m1 = matrix;
    ...
    matrix = malloc((*m2_rows) * (*m2_cols) * sizeof(double));

    for (i = 0; i < *m2_rows; i++) {
        for (j = 0; j < *m2_cols; j++) {
            fscanf(fp, "%lf", &matrix[j + i*(*m2_cols)]);
        }
    }
