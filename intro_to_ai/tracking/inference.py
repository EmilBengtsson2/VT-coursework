...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for entry in beliefDistribution:
                dist = util.manhattanDistance(pacmanPosition, entry)
                updatedBeliefs[entry] = beliefDistribution[entry] * emissionModel[dist]
            if updatedBeliefs.totalCount() == 0:
                self.initializeUniformly(gameState)
                updatedBeliefs = self.getBeliefDistribution()
            self.particles = []
            for i in range(self.numParticles):
                self.particles.append(util.sample(updatedBeliefs))
    ...
    def elapseTime(self, gameState):
        ...
        newBeliefs = util.Counter()
        for oldPos in self.legalPositions:
            if beliefDistribution[oldPos] == 0:
                continue
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos in self.legalPositions:
                if newPosDist[newPos] == 0:
                    continue
                newBeliefs[newPos] += newPosDist[newPos] * beliefDistribution[oldPos]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = []
        i = 0
        product = [entry for entry in itertools.product(self.legalPositions, repeat=self.numGhosts)]
        while i < self.numParticles:
    ...
    def observeState(self, gameState):
        ...
        beliefDistribution = self.getBeliefDistribution()
        updatedBeliefs = beliefDistribution.copy()
        for i in range(self.numGhosts):
            if not noisyDistances[i] is None:
                for entry in beliefDistribution:
                    dist = util.manhattanDistance(pacmanPosition, entry[i])
                    updatedBeliefs[entry] *= emissionModels[i][dist]
        if updatedBeliefs.totalCount() == 0:
            self.initializeParticles()
            updatedBeliefs = self.getBeliefDistribution()
        self.particles = []
